...
void findEyes(cv::Mat frame_gray, cv::Rect face) {
  ...
  output_path << output_dir << "/detected/detect_" << std::setw(5)
              << std::setfill('0') << counter << ".png";
  cv::resize(frame, frame, cv::Size(0, 0), (1.0 * faceROI.rows) / frame.rows,
             (1.0 * faceROI.rows) / frame.rows);
...
#if WRITE_OUT_IMAGES
  std::ostringstream output_path;
  output_path << output_dir << "/images/frame_" << std::setw(5)
              << std::setfill('0') << counter << ".png";
